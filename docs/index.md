...
- **MCP server** (`/api/mcp`) — the agent's interface. 46 typed tools for reading and writing every piece of productivity data, 13 prompt templates, plus read-only resources. Bearer-token authenticated.
...
-   :material-api:{ .lg .middle } **MCP reference**
    ...
    46 tools, grouped by domain, with input schemas and examples.
