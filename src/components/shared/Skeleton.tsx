...
export function DashboardSkeleton() {
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-8">
      ...
      <Skeleton width="260px" height="14px" className="mb-2" />
      <Skeleton width="220px" height="12px" className="mb-8" />
      <Skeleton variant="rectangle" height="140px" className="mb-5" />
      <Skeleton variant="rectangle" height="72px" className="mb-5" />
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {Array.from({ length: 6 }).map((_, i) => (
          <Skeleton key={i} variant="rectangle" height="128px" />
        ))}
      </div>
    </div>
  );
