...
export async function executeTool(
  ...
  switch (toolName) {
    ...
    case "search_conversations": {
      ...
      // Get user's conversation IDs, optionally filtered by date
      let convQuery = supabase
        .from("conversations")
        .select("id")
        .eq("user_id", userId);

      if (dateStart) convQuery = convQuery.gte("created_at", dateStart);
      if (dateEnd) convQuery = convQuery.lte("created_at", dateEnd + "T23:59:59");

      const { data: userConvs } = await convQuery;
      const convIds = (userConvs || []).map((c) => c.id);

      if (convIds.length === 0) {
        return { success: true, summary: "No conversations found in the specified date range.", data: [] };
      }

      // Full-text search on messages
      const { data: ftsMessages, error: ftsError } = await supabase
        .from("messages")
      ...
      if (ftsError) {
        return { success: false, summary: `Search failed: ${ftsError.message}`, error: ftsError.message };
      }

      if (!ftsMessages || ftsMessages.length === 0) {
        return { success: true, summary: `No past conversations found matching "${query}".`, data: [] };
      }

      // Look up conversation titles
      ...
      // Build snippets (~200 chars around match, no HTML tags)
      const generateSnippet = (content: string, searchQuery: string): string => {
      ...
      const results = ftsMessages.map((msg) => ({
      ...
      // If search model is configured, summarize the results
      if (context?.searchModel) {
        try {
          const { complete: llmComplete } = await import("@/lib/llm");
          const resultsText = results.map((r, i) =>
            `[${i + 1}] "${r.conversationTitle}" (${r.role}, ${r.date}):\n${r.snippet}`
          ).join("\n\n");

          const summaryResponse = await llmComplete(context.searchModel, {
            messages: [
              {
                role: "system",
                content: "Summarize these search results from past chat conversations. Be concise and highlight the most relevant findings. Focus on what was discussed, not metadata.",
              },
              {
                role: "user",
          ...
          if (summaryText) {
            return { success: true, summary: summaryText, data: results };
          }
        } catch (err) {
          console.error("Search summarization failed, falling back to raw results:", err);
        }
      }

      // Raw snippet listing
      const summaryLines = results.map((r) =>
        `- "${r.conversationTitle}" (${r.role}, ${r.date}): ${r.snippet}`
      ).join("\n");
